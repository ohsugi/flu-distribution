...
global {
	...
	file park_shapefile <- file("../includes/parks.shp");
	file buildings_shapefile <- file("../includes/buildings.shp");
	geometry shape <- envelope(envelope(roads_shapefile) + envelope(buildings_shapefile) + envelope(park_shapefile));
	graph road_network;
	...
	init{
		create park from: park_shapefile;
		create road from: roads_shapefile where (each != nil);
		road_network <- as_edge_graph(road);		
		create building from: buildings_shapefile
			with: [elementId::int(read('full_id')), elementHeight::int(read('Height')), elementBuilding::string(read('building'))];
...
species building {
    int elementId;
    int elementHeight;
    string elementBuilding;
	aspect geom {
		if (elementBuilding = "retail") {
    		draw shape color: rgb("yellow", 180) depth: elementHeight;
		} else if (elementBuilding = "school") {
    		draw shape color: rgb("black", 180) depth: elementHeight;
		} else if (elementBuilding = "public") {
    		draw shape color: rgb("blue", 180) depth: elementHeight;
		} else {
    		draw shape color: rgb("gray", 180) depth: elementHeight;
		}
	}
	aspect geom3D {
...
species park {
	aspect geom {
        draw shape color: rgb("yellow", 100);
    }
	aspect geom3D {
		draw shape depth: 1 #m color: rgb("yellow", 100);
	}
} 
